...
`--no-provenance` is required: `publishConfig.provenance` is `true`, and provenance can only
be generated by a CI runner with an OIDC token. Without the flag the publish is refused.
